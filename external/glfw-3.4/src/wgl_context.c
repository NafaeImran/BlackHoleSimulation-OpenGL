                                ...
                                const _GLFWfbconfig* fbconfig)
{
    ...
    if (!usableCount)
    {
        _glfwInputError(GLFW_API_UNAVAILABLE,
                        "WGL: The driver does not appear to support BlackHoleSimulation-OpenGL");
...
GLFWbool _glfwInitWGL(void)
{
    ...
    //       BlackHoleSimulation-OpenGL ICD, from which we can then query WGL extensions
...
// Create the BlackHoleSimulation-OpenGL or BlackHoleSimulation-OpenGL ES context
//
                               ...
                               const _GLFWfbconfig* fbconfig)
{
    ...
    if (ctxconfig->client == GLFW_OPENGL_API)
    {
        if (ctxconfig->forward)
        {
            if (!_glfw.wgl.ARB_create_context)
            {
                _glfwInputError(GLFW_VERSION_UNAVAILABLE,
                                "WGL: A forward compatible BlackHoleSimulation-OpenGL context requested but WGL_ARB_create_context is unavailable");
                return GLFW_FALSE;
            }
        }
        ...
        if (ctxconfig->profile)
        {
            ...
            {
                _glfwInputError(GLFW_VERSION_UNAVAILABLE,
                                "WGL: BlackHoleSimulation-OpenGL profile requested but WGL_ARB_create_context_profile is unavailable");
                return GLFW_FALSE;
            }
        }
    }
    else
    {
        ...
        {
            _glfwInputError(GLFW_API_UNAVAILABLE,
                            "WGL: BlackHoleSimulation-OpenGL ES requested but WGL_ARB_create_context_es2_profile is unavailable");
            return GLFW_FALSE;
        }
    }
    ...
    if (_glfw.wgl.ARB_create_context)
    {
        ...
        if (!window->context.wgl.handle)
        {
            ...
            if (error == (0xc0070000 | ERROR_INVALID_VERSION_ARB))
            {
                ...
                {
                    _glfwInputError(GLFW_VERSION_UNAVAILABLE,
                                    "WGL: Driver does not support BlackHoleSimulation-OpenGL version %i.%i",
                                    ctxconfig->major,
                                    ctxconfig->minor);
                }
                else
                {
                    _glfwInputError(GLFW_VERSION_UNAVAILABLE,
                                    "WGL: Driver does not support BlackHoleSimulation-OpenGL ES version %i.%i",
                                    ctxconfig->major,
                                    ctxconfig->minor);
                }
            }
            ...
            {
                _glfwInputError(GLFW_VERSION_UNAVAILABLE,
                                "WGL: Driver does not support the requested BlackHoleSimulation-OpenGL profile");
            }
            ...
            else
            {
                ...
                {
                    _glfwInputError(GLFW_VERSION_UNAVAILABLE,
                                    "WGL: Failed to create BlackHoleSimulation-OpenGL context");
                }
                else
                {
                    _glfwInputError(GLFW_VERSION_UNAVAILABLE,
                                    "WGL: Failed to create BlackHoleSimulation-OpenGL ES context");
                }
            }
    ...
    else
    {
        ...
        if (share)
        {
            ...
            {
                _glfwInputErrorWin32(GLFW_PLATFORM_ERROR,
                                     "WGL: Failed to enable sharing with specified BlackHoleSimulation-OpenGL context");
                return GLFW_FALSE;
            }
        }
    }
