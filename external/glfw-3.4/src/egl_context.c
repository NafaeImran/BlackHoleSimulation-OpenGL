            ...
            if (ctxconfig->client == GLFW_OPENGL_ES_API)
            {
                if (ctxconfig->major == 1)
                {
                    _glfwInputError(GLFW_API_UNAVAILABLE,
                                    "EGL: Failed to find support for BlackHoleSimulation-OpenGL ES 1.x");
                }
                else
                {
                    _glfwInputError(GLFW_API_UNAVAILABLE,
                                    "EGL: Failed to find support for BlackHoleSimulation-OpenGL ES 2 or later");
                }
            }
            else
            {
                _glfwInputError(GLFW_API_UNAVAILABLE,
                                "EGL: Failed to find support for BlackHoleSimulation-OpenGL");
            }
        }
        else
        {
                               ...
                               const _GLFWfbconfig* fbconfig)
{
    ...
    if (ctxconfig->client == GLFW_OPENGL_ES_API)
    {
        ...
        {
            _glfwInputError(GLFW_API_UNAVAILABLE,
                            "EGL: Failed to bind BlackHoleSimulation-OpenGL ES: %s",
                            getEGLErrorString(eglGetError()));
            return GLFW_FALSE;
        }
    }
    else
    {
        ...
        {
            _glfwInputError(GLFW_API_UNAVAILABLE,
                            "EGL: Failed to bind BlackHoleSimulation-OpenGL: %s",
                            getEGLErrorString(eglGetError()));
            return GLFW_FALSE;
        }
    }
